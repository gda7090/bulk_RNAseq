...
qsigr <- read.table(opt$qsigr1,header = T,row.names = 1,sep="\t")
qsigr[qsigr==0]<-NA
...
if(!is.null(opt$anno2)){
	...
	groups<-unique(ha_col[,1])
	colors<-brewer.pal(length(groups),"Set2")
	names(colors)<-groups
	ha_col<- HeatmapAnnotation(ha_col,col = list(group=colors))
#	c("NC" = "black","HT" = "red", "NGD" = "green", "RGD" = "blue")))
...
if(!is.null(opt$top)){
    top<-ifelse(nrow(qsigr)>opt$top,opt$top,nrow(qsigr))
#    qsigr <- corr1[row.names(qsigr),colnames(qsigr)]
    qsigr <- qsigr[order(rowSums(abs(qsigr),na.rm=T),decreasing = T)[1:top],]
...
#color<-col=colorRampPalette(c("#2f98ce", "#e0f3db", "#f47104"))
color <- colorRamp2(seq(min(qsigr,na.rm=T), max(qsigr,na.rm=T), length = 2), c("white", "red"), space = "RGB")
...
ht1 = Heatmap(qsigr,  col = color,  
    ...
    heatmap_legend_param = list(legend_height=unit(4,"cm"), legend_direction="vertical",title = ""),
...
svg(file=paste(outdir,opt$prefix,"_heatmap.svg",sep=""),width = ncol(qsigr)/5+2+namelen_row/30, height = nrow(qsigr)/5+2+namelen_col/30)
ht1
dev.off()
