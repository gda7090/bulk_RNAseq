...
for( i in 1:length(matfiles)){
	outdir<-temp.outdir
	outdir<-paste(outdir,matfiles[i],sep="") #Level[cfgops] like " otu_table.c.absolute.mat ",so11 corresponding to  o s f p ...
	if (!file.exists(outdir)){
		dir.create(outdir)
	}
	data<-read.table(paste(infilepath,"/",matfiles[i],sep=""),head=T,sep="\t", row.names=1)
#	data<-T.level
...
#	data <- data[which(apply(data ,1,sum)!=0),] #过滤全0行
	...
	for(p in 1:length(Pairs)){
        ...
        if(length(group1)>2 & length(group2)>2){
	    	...
	    	write.table(data[,group.pair],paste(pair.file.name,".mat",sep=""),quote = FALSE,sep="\t",col.names = NA)
            ...
            if(dim(allsame)[1]>0){
                ...
                write.table(allsame,file=paste(pair.file.name,".mat.same.xls",sep=""),row.names = F,quote = FALSE,sep = "\t");
            }
