...
my $lib = "$Bin/../../../00.Commbin/";
use PATHWAY;
(-s "$Bin/../../../../bin/Pathway_cfg.txt") || die"error: can't find config at $Bin/../../../../bin/Pathway_cfg.txt, $!\n";

BEGIN{
        my($math_comb) = get_pathway("$Bin/../../../../bin/Pathway_cfg.txt",[qw(MATH_COMB)],$Bin,$lib);                                                                       
...
use Data::Dumper;
use File::Basename;
my($R,$convert) = get_pathway("$Bin/../../../../bin/Pathway_cfg.txt",[qw(R4 CONVERT)],$Bin,$lib);
my %opt=("outdir" => ".");
